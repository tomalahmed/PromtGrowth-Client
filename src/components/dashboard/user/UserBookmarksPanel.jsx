...
export default function UserBookmarksPanel() {
  ...
  return (
    <>
      <UserPageHeader
        title="Saved Prompts"
        subtitle="Prompts you've bookmarked from the marketplace."
      />
      ...
      {isLoading ? (
        ...
        >
          ...
          <p className="mt-2 max-w-sm text-[14px] text-on-surface-variant">
            Browse the marketplace and bookmark prompts you want to use later.
          </p>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
          {prompts.map((prompt, index) => (
            <BookmarkCard key={prompt._id} prompt={prompt} index={index} />
          ))}
        </div>
      )}
    </>
...
function BookmarkCard({ prompt, index }) {
